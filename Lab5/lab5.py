...
class Distribution:
    def __init__(self, row=[1, 2, 2, 2, 1, 4, 4, 5, 5, 5]):
        ...
        # self.create_relative_frequency_polygon()

    def read_row(self):
        return [float(i) for i in open("z1.txt").readline().split(" ")]

    def get_var_row(self):
        return sorted(self.row)

    def get_stat_frequency(self):
        return dict(Counter(self.var_row))

    def get_stat_relative_frequency(self):
        return {key: value / self.n for key, value in self.stat_frequency.items()}
    ...
    def get_distribution_func(self):
        ...
        for value, relative_frequency in self.stat_relative_frequency.items():
            distribution_func[frequency] = (minimum, value)
            minimum = value
    ...
    def get_x_selective(self):
        return sum([value * frequency for value, frequency in self.stat_frequency.items()]) / self.n

    def get_variance(self):
        return sum([frequency * (value - self.x_selective) ** 2 for value, frequency in self.stat_frequency.items()]) / self.n
    ...
    def create_func_graph(self):
        ...
        a = min(list(self.stat_frequency.keys())[0], 0) - 2
        b = list(self.stat_frequency.keys())[0]
        ax.plot((min(list(self.stat_frequency.keys())[0], 0) - 2, list(self.stat_frequency.keys())[0]), (0, 0), color='b')
        for part in list(self.distribution_func.items())[1:-1]:
            x1, y1 = (part[1][0], part[0])
            x2, y2 = part[1][1], part[0]
            ax.plot((x1, x2), (y1, y2), color='b')
        ax.plot((list(self.stat_frequency.keys())[-1], list(self.stat_frequency.keys())[-1] + 2), (1, 1), color='b')
        x_axes = [i[0] for i in self.distribution_func.values()]
        ax.plot(x_axes[1:], list(self.distribution_func.keys())[1:], 'bo')
        ax.plot(x_axes[1:], list(self.distribution_func.keys())[1:], 'w.')
    ...
    def create_frequency_polygon(self):
        fig, ax = plt.subplots(1, 1)
        x = list(self.stat_frequency.keys())
    ...
    def create_relative_frequency_polygon(self):
        fig, ax = plt.subplots(1, 1)
        x = list(self.stat_relative_frequency.keys())
...
def show_frequencies_row(dist: Distribution):
    ...
    x.add_row(["Ni/N"] + [str(round(i, 3)) for i in dist.stat_relative_frequency.values()])
    print(x.get_string(header=False))
